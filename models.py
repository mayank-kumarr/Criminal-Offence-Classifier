...
class COClassifier(nn.Module):
    def __init__(self, vocab_size, embed_dim, num_labels,
                 offence, charge_sent_lens, charge_doc_lens,
        ...
        if sent_label_wts is not None:
            self.sent_label_wts = sent_label_wts
        else:
            self.sent_label_wts = torch.ones((self.num_labels,), device=self.device)
        if doc_label_wts is not None:
            self.doc_label_wts = doc_label_wts
        else:
            self.doc_label_wts = torch.ones((self.num_labels,), device=self.device)
